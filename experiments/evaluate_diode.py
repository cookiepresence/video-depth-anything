...
def plot_imgs(src, pred, gt, mask, name):
    ...
    def normalize_tensor(tensor, valid_mask):
        ...
        if valid_vals.size > 0:
            ...
            norm_arr = np.zeros_like(arr)
        norm_arr[~msk] = 0  # Optional: black out invalid areas
    ...
    axs[1].imshow(pred.cpu().numpy(), cmap='plasma')
    axs[1].set_title("Predicted Depth")
    axs[2].imshow(gt.cpu().numpy(), cmap='plasma')
    axs[2].set_title("Ground Truth Depth")
    axs[3].imshow(torch.abs(pred - gt).cpu().numpy(), cmap='plasma')
...
def evaluate_vda(model, dataset):
    ...
    with torch.no_grad():
        for batch_num, (imgs, disparity_gt, masks) in tqdm.tqdm(enumerate(dataset)):
            ...
            # dim: [B, S, C, H, W]
            disparity_preds = model(imgs)
            # currently: calculate evals over disparity
            # ideally: disparity -> (pseudo)-depth, evals over depth range
            # [b, s, h, w] -> [n, h, w]
            batch_size = imgs.shape[0]
            disparity_gt = einops.rearrange(disparity_gt, 'b s h w -> (b s) h w')
            disparity_preds = einops.rearrange(disparity_preds, 'b s h w -> (b s) h w')
            masks = einops.rearrange(masks, 'b s h w -> (b s) h w')
            ...
            for i in range(disparity_gt.shape[0]):
                ...
                if batch_metrics['abs_rel'] > 1:
                    plot_imgs(imgs[i], disparity_preds[i], disparity_gt[i], masks[i], batch_num * 32 + i)
                    print(batch_metrics)
...
vda = load_vda(model_name='vitl', model_weights=Path('checkpoints/video_depth_anything_vitl.pth'), backbone_weights=None)

# TODO: make sure this is correctly cropped to ensure that
# the model works correctly
eval_dataset = diode.DIODE(
    data_root='/scratch/mde/diode',
    meta_fname='datasets/diode_meta.json',
    splits='val')
