...
def plot_imgs(src, pred, gt, mask, name):
    ...
    axs[2].imshow(gt.cpu().numpy(), cmap='plasma', vmax=90)
    axs[2].set_title("Ground Truth Depth")
    axs[3].imshow(torch.abs(pred - gt).cpu().numpy(), cmap='plasma', vmax=90)
    ...
    with torch.no_grad():
        for batch_num, (imgs, disparity_gt, masks) in tqdm.tqdm(enumerate(dataset)):
            ...
            with torch.autocast(device_type=DEVICE, dtype=torch.float16, enabled=True):
                disparity_preds = model(imgs, over_imgs=True)
            ...
            for i in range(disparity_gt.shape[0]):
                ...
                if batch_metrics['abs_rel'] > 0.3 or True: # TODO: remove this
...
eval_dataset = kitti.KITTIDepth(
    data_root='/scratch/mde/kitti_depth',
    splits='val',
    download = True
)
