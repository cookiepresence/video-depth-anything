...
def TemporalGradientMatchingLoss(pred, target, mask=None):
    assert pred.shape == target.shape, f"pred and target must have the same shape, pred: {pred.shape}, target: {target.shape}"
    ...
    # ensures that we don't compute losses over very large differences
    ...
    temporal_gm_loss = torch.nn.functional.l1_loss(pred_diff * time_mask, target_diff * time_mask)

    return temporal_gm_loss / (num_frames- 1)
...
def feature_distillation_loss(student_features, teacher_features, adaptation_layers=None, weights=None):
    ...
    for i, (s_feat, t_feat, weight) in enumerate(zip(student_features, teacher_features, weights)):
        ...
        assert (s_feat.shape == t_feat.shape), f"student and teacher feature maps have different shapes! (student: {s_feat.shape} vs teacher: {t_feat.shape})" # 
...
def distillation_train_step(
    ...
    for imgs, depth_maps, metadata in tqdm.tqdm(train_dataloader):
        ...
        for idx, (sb_imgs, sb_depths, sb_teacher_features) in enumerate(zip(sub_batch_imgs, sub_batch_depths, sub_batch_teacher_features)):
            ...
            temporal_loss = TemporalGradientMatchingLoss(student_output, teacher_output[:, idx * sub_batch_size:(idx + 1) * sub_batch_size, :, :].squeeze(0))
            
            # Combined loss for this sub-batch (scaled by sub-batch size proportion)
            sub_batch_loss = (1 - distill_lambda) * depth_loss + distill_lambda * distill_loss # + temporal_loss
            print(f"Temporal Loss: {temporal_loss} | Distill Loss: {distill_loss} | Subbatch Loss: {sub_batch_loss}")
            ...
            temporal_batch_loss += temporal_loss.item()
        ...
        train_loss.update({
            ...
            "temporal_loss": temporal_batch_loss
        })
